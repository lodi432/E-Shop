...
if(isset($_GET['edit'])){
    ...
    $productResults->execute();
      ...
      if (isset($_GET['delete_image'])){
        $imgi = (int)$_GET['imgi'] -0;
        $images = explode(',',$product['image']);
      $image_url = $_SERVER['DOCUMENT_ROOT'].$images[$imgi];
      unlink($image_url);
      unset($images[$imgi]);
      $imageString=implode(',',$images);
      $izrazDel= $veza->prepare("UPDATE products SET image = '{$imageString}'WHERE id = '$edit_id';");
      $izrazDel->execute();
  ...
  <?php if($saved_image != ''):?>
    <?php
      ...
      <?php foreach($images as $image) : ?>
        <br>
    <div class="saved-image columns medium-6" >
       <img src="<?=$image;?>"alt="saved image"/><br><br>
        <a href="products.php?delete_image=1&edit=<?=$edit_id;?>&imgi=<?$imgi;?>" class="button label alert">Delete Image</a>
     </div>
  <?php
         $imgi++;
endforeach;?>
...
<div class="app-dashboard shrink-medium">
  ...
  <div class="app-dashboard-body off-canvas-wrapper">
    ...
    <div class="app-dashboard-body-content off-canvas-content" data-off-canvas-content>
      ...
      <h3 class="text">Products</h2>

      <center><a href="products.php?add=1" class="button expanded float-center callout clearfix" id=""><strong>Dodaj Novi Proizvod </strong> <i class="fas fa-angle-right "></i></a>
      <hr>
